...
final class Version20260405040000 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TABLE lab_task (
        ...
        $this->addSql('CREATE INDEX IDX_lab_task_lab ON lab_task (lab_id)');
        $this->addSql('ALTER TABLE lab_task ADD CONSTRAINT FK_lab_task_lab FOREIGN KEY (lab_id) REFERENCES lab (id) ON DELETE CASCADE');

        // 2. Create lab_task_compliance_policy join table
        $this->addSql('CREATE TABLE lab_task_compliance_policy (
            lab_task_id INT NOT NULL,
            compliance_policy_id INT NOT NULL,
            PRIMARY KEY (lab_task_id, compliance_policy_id)
        )');
        $this->addSql('CREATE INDEX IDX_ltcp_task ON lab_task_compliance_policy (lab_task_id)');
        $this->addSql('CREATE INDEX IDX_ltcp_policy ON lab_task_compliance_policy (compliance_policy_id)');
        $this->addSql('ALTER TABLE lab_task_compliance_policy ADD CONSTRAINT FK_ltcp_task FOREIGN KEY (lab_task_id) REFERENCES lab_task (id) ON DELETE CASCADE');
        $this->addSql('ALTER TABLE lab_task_compliance_policy ADD CONSTRAINT FK_ltcp_policy FOREIGN KEY (compliance_policy_id) REFERENCES compliance_policy (id) ON DELETE CASCADE');

        // 3. Migrate existing data: create a default task per lab and move policies
        $this->addSql("
