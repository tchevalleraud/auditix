...
class CollectionRuleController extends AbstractController
{
    ...
    private function serializeFolder(CollectionRuleFolder $f, EntityManagerInterface $em): array
    {
        ...
        return [
            ...
            'managedByPlugin' => $f->getManagedByPlugin(),
    ...
    public function update(CollectionRule $rule, Request $request, EntityManagerInterface $em): JsonResponse
    {
        ...
        if ($rule->isManagedByPlugin()) {
            // Plugin-managed rules are read-only except for the `enabled` toggle.
            ...
            return $this->json(
                ['error' => sprintf('This rule is managed by the "%s" plugin and cannot be edited (only enabled/disabled).', $rule->getManagedByPlugin())],
    ...
    public function delete(CollectionRule $rule, EntityManagerInterface $em): JsonResponse
    {
        ...
        if ($rule->isManagedByPlugin()) {
            return $this->json(
                ['error' => sprintf('This rule is managed by the "%s" plugin. Disable the plugin to remove it.', $rule->getManagedByPlugin())],
                Response::HTTP_FORBIDDEN,
            );
        }
        $em->remove($rule);
