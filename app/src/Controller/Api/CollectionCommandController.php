...
class CollectionCommandController extends AbstractController
{
    private function serializeCommand(CollectionCommand $c): array
    {
        return [
            ...
            'managedByPlugin' => $c->getManagedByPlugin(),
    ...
    public function update(CollectionCommand $cmd, Request $request, EntityManagerInterface $em): JsonResponse
    {
        ...
        if ($cmd->isManagedByPlugin()) {
            // Plugin-managed commands are read-only except for the `enabled` toggle.
            if (isset($data['enabled'])) {
                $cmd->setEnabled((bool) $data['enabled']);
                $em->flush();
                return $this->json($this->serializeCommand($cmd));
            }
            return $this->json(
                ['error' => sprintf('This command is managed by the "%s" plugin and cannot be edited (only enabled/disabled).', $cmd->getManagedByPlugin())],
    ...
    public function delete(CollectionCommand $cmd, EntityManagerInterface $em): JsonResponse
    {
        ...
        if ($cmd->isManagedByPlugin()) {
            return $this->json(
                ['error' => sprintf('This command is managed by the "%s" plugin. Disable the plugin to remove it.', $cmd->getManagedByPlugin())],
                Response::HTTP_FORBIDDEN,
            );
        }
        $em->remove($cmd);
