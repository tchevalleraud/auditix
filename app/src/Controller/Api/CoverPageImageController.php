...
#[Route('/api/cover-page-images')]
class CoverPageImageController extends AbstractController
{
    #[Route('', methods: ['POST'])]
    public function upload(Request $request): JsonResponse
    {
        ...
        $uploadDir = $this->getParameter('kernel.project_dir') . '/var/uploads/cover-pages';
        if (!is_dir($uploadDir)) {
            mkdir($uploadDir, 0775, true);
        }

        $extension = $file->guessExtension() ?? 'png';
        $filename = uniqid('cp-') . '.' . $extension;
        $file->move($uploadDir, $filename);

        return $this->json([
            'filename' => $filename,
            'url' => '/api/cover-page-images/' . $filename,
        ]);
    }
    ...
    public function serve(string $filename): Response
    {
        ...
        $path = $this->getParameter('kernel.project_dir') . '/var/uploads/cover-pages/' . $filename;
