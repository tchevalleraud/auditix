...
#[Route('/api/report-schema-folders')]
class ReportSchemaFolderController extends AbstractController
{
    #[Route('', methods: ['POST'])]
    public function create(Request $request, EntityManagerInterface $em): JsonResponse
    {
        ...
        if (!empty($data['parentId'])) {
            $parent = $em->getRepository(ReportSchemaFolder::class)->find($data['parentId']);
            if ($parent && $parent->getContext()->getId() !== $context->getId()) {
                $parent = null;
            }
        }

        $folder = new ReportSchemaFolder();
        $folder->setName($name);
    ...
    public function update(ReportSchemaFolder $folder, Request $request, EntityManagerInterface $em): JsonResponse
    {
    ...
    public function delete(ReportSchemaFolder $folder, EntityManagerInterface $em): JsonResponse
    {
        $this->denyAccessUnlessGranted(ContextAccessVoter::ACCESS, $folder->getContext());

        // Subfolders cascade (onDelete CASCADE); schemas are detached to root
        // (ReportSchema.folder onDelete SET NULL), so no schema is lost.
        $em->remove($folder);
        $em->flush();
    ...
    private function wouldCreateCycle(ReportSchemaFolder $folder, ReportSchemaFolder $parent): bool
    {
        $cursor = $parent;
        while ($cursor !== null) {
