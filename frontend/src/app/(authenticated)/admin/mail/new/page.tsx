...
export default function NewMailServerPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    router.push(`/admin/mail/${created.id}/edit`);
  };

  return (
    <div className="space-y-6">
      <div>
        <Link href="/admin/mail" className="inline-flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors mb-3">
      ...
      <form onSubmit={submit} className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-6 space-y-4">
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
          ...
          <Link href="/admin/mail" className="rounded-lg border border-slate-200 dark:border-slate-700 px-5 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            {t("admin_mail.cancel")}
          </Link>
        </div>
      </form>
