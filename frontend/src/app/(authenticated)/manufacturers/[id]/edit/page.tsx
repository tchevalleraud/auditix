...
export default function EditEditorPage() {
  ...
  useEffect(() => {
    fetch(`/api/manufacturers?context=${current?.id}`)
      ...
      .then((editors: EditorItem[]) => {
        const found = editors.find((e) => e.id === Number(editorId));
        if (found) {
          setEditor(found);
          setName(found.name);
          setDescription(found.description ?? "");
          if (found.logo) setLogoPreview(found.logo);
        }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/manufacturers/${editorId}`, {
        method: "POST",
        body: formData,
      });
