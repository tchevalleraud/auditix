...
export default function NewContextPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        await reload();
        router.push("/settings/global/contexts");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-start justify-between gap-4">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{t("admin_contexts.newContext")}</h1>
          <button
            type="submit"
            form="settings-form-context-new"
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
        <form id="settings-form-context-new" onSubmit={handleSubmit} className="p-6 space-y-5">
