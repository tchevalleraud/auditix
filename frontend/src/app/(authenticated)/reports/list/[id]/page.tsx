...
export default function ReportDetailPage() {
  ...
  const handleReset = async () => {
    setResetting(true);
    try {
      const res = await fetch(`/api/reports/${reportId}/reset`, {
        method: "POST",
      });
      if (res.ok) {
        const data = await res.json();
    ...
    <div className="flex flex-col gap-6 h-[calc(100vh-7rem)]">
        ...
        <div className="flex items-center gap-2">
          {isGenerating && (
            <button
              ...
              disabled={resetting}
              title={t("reports.resetHint")}
              className="flex items-center gap-2 rounded-lg border border-slate-200 dark:border-slate-700 px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-50 transition-colors"
            >
              {resetting ? (
      ...
      {resetConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-sm p-6 space-y-4">
            <p className="text-sm text-slate-700 dark:text-slate-300">
              {t("reports.confirmReset")}
            </p>
            <div className="flex items-center justify-end gap-2">
              <button
                onClick={() => setResetConfirm(false)}
                disabled={resetting}
                className="rounded-lg px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-50 transition-colors"
              >
                {t("common.cancel")}
              </button>
              <button
